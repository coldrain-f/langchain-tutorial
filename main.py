...
def generate_chat_completion(message: str, model: str):
    ...
    return model.invoke(message).content, model.stream(message)
...
if user_message:
    ...
    chat_invoke_completion, chat_stream_completion = generate_chat_completion(
        user_message, selected_model
    )
    add_message("assistant", chat_invoke_completion)
    st.chat_message("assistant").write_stream(chat_stream_completion)
